...
// The vigil: presence for /root/. Owns identity, the heartbeat, the right-edge
// presence stack, and the drifting ghosts. Clicking a presence chip decodes it
// in the terminal — forwarded through a holder the terminal fills in once it
// exists (so the decode output lands in the log via the `decode` command).
let runDecode = () => {};
const vigil = createVigil({
  flare: rain.flare,
  audio,
  mount: document.getElementById("vigil"),
  decodeId: (id) => runDecode(id),
});
