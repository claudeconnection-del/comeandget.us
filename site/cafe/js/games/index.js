...
export function gameCommands() {
  const cmds = {
    ...
    "4k": ({ argv, api }) => {
      if (!is4kUnlocked()) {
        api.print(api.sp("✦ 4K isn't poured yet — ", "c-muted"), api.sp("finish any game once (or clear a Tetris)", "c-text"), api.sp(" and it's yours: neon, particles, a little shake, crisper & bigger.", "c-muted"));
        return;
      }
